report 50202 "Sales Proforma Invoive Report"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Sales Document - Test.rdl';
    ...
    dataset
    {
        dataitem("Sales Header"; "Sales Header")
        {
            ...
            column(Document_Caption; DocumentCaption)
            {
            }
            column(QMS_No; IDNo)
            {
            }
            column(QMSIDCaption; QMSIDCaption)
            {
            }
            column(IssueNo; IssueNo)
            {
            }
            column(Issue_Date_Caption; IssueDateCaption)
            {
            }
                ...
                dataitem(CopyLoop; "Integer")
                {
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        begin
                            ...
                            with "Sales Line" do begin
                                if not "Sales Header"."Prices Including VAT" and ("VAT Calculation Type" = "VAT Calculation Type"::"Full VAT") then TempSalesLine."Line Amount" := 0;
                                DimSetEntry2.SetRange("Dimension Set ID", "Dimension Set ID");
                                DimMgt.GetDimensionSet(TempDimSetEntry, "Dimension Set ID");
                                if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then begin
                                    if "Document Type" = "Document Type"::"Credit Memo" then begin
                                        if ("Return Qty. to Receive" <> Quantity) and ("Return Receipt No." = '') then AddError(StrSubstNo(Text015, FieldCaption("Return Qty. to Receive"), Quantity));
                                        if "Qty. to Invoice" <> Quantity then AddError(StrSubstNo(Text015, FieldCaption("Qty. to Invoice"), Quantity));
                                    end;
                                    if "Qty. to Ship" <> 0 then AddError(StrSubstNo(Text043, FieldCaption("Qty. to Ship")));
                                end
                                else begin
                                    if "Document Type" = "Document Type"::Invoice then begin
                                        if ("Qty. to Ship" <> Quantity) and ("Shipment No." = '') then AddError(StrSubstNo(Text015, FieldCaption("Qty. to Ship"), Quantity));
                                        if "Qty. to Invoice" <> Quantity then AddError(StrSubstNo(Text015, FieldCaption("Qty. to Invoice"), Quantity));
                                    end;
                                    if "Return Qty. to Receive" <> 0 then AddError(StrSubstNo(Text043, FieldCaption("Return Qty. to Receive")));
                                end;
                                if not "Sales Header".Ship then "Qty. to Ship" := 0;
                                if not "Sales Header".Receive then "Return Qty. to Receive" := 0;
                                ...
                                if "Sales Header".Invoice then begin
                                    ...
                                    if Abs("Qty. to Invoice") > Abs(MaxQtyToBeInvoiced) then "Qty. to Invoice" := MaxQtyToBeInvoiced;
                                end
                                else
                                    "Qty. to Invoice" := 0;
                                if "Gen. Prod. Posting Group" <> '' then begin
                                    if ("Sales Header"."Document Type" in ["Sales Header"."Document Type"::"Return Order", "Sales Header"."Document Type"::"Credit Memo"]) and ("Sales Header"."Applies-to Doc. Type" = "Sales Header"."Applies-to Doc. Type"::Invoice) and ("Sales Header"."Applies-to Doc. No." <> '') then begin
                                        ...
                                        if (not CustLedgEntry.FindLast) and (not ApplNoError) then begin
                                            ApplNoError := true;
                                            AddError(StrSubstNo(Text016, "Sales Header".FieldCaption("Applies-to Doc. No."), "Sales Header"."Applies-to Doc. No."));
                                        end;
                                    end;
                                    if not VATPostingSetup.Get("VAT Bus. Posting Group", "VAT Prod. Posting Group") then AddError(StrSubstNo(Text017, VATPostingSetup.TableCaption, "VAT Bus. Posting Group", "VAT Prod. Posting Group"));
                                    if VATPostingSetup."VAT Calculation Type" = VATPostingSetup."VAT Calculation Type"::"Reverse Charge VAT" then
                                        if ("Sales Header"."VAT Registration No." = '') and (not VATNoError) then begin
                                            VATNoError := true;
                                            AddError(StrSubstNo(Text035, "Sales Header".FieldCaption("VAT Registration No.")));
                                        end;
                                end;
                                if Quantity <> 0 then begin
                                    if "No." = '' then AddError(StrSubstNo(Text019, Type, FieldCaption("No.")));
                                    if Type = Type::" " then AddError(StrSubstNo(Text006, FieldCaption(Type)));
                                end
                                else
                                    if Amount <> 0 then AddError(StrSubstNo(Text020, FieldCaption(Amount), FieldCaption(Quantity)));
                                if "Drop Shipment" then begin
                                    if Type <> Type::Item then AddError(Text021);
                                ...
                                if (Type <> Type::" ") and ("Qty. to Invoice" <> 0) then begin
                                    if not ApplicationAreaMgmt.IsSalesTaxEnabled then if not GenPostingSetup.Get("Gen. Bus. Posting Group", "Gen. Prod. Posting Group") then AddError(StrSubstNo(Text017, GenPostingSetup.TableCaption, "Gen. Bus. Posting Group", "Gen. Prod. Posting Group"));
                                    if not VATPostingSetup.Get("VAT Bus. Posting Group", "VAT Prod. Posting Group") then AddError(StrSubstNo(Text017, VATPostingSetup.TableCaption, "VAT Bus. Posting Group", "VAT Prod. Posting Group"));
                                end;
                                if "Prepayment %" > 0 then
                                    if not "Prepayment Line" and (Quantity > 0) then begin
                                        ...
                                        case true of
                                            (Fraction * "Line Amount" < "Prepmt Amt to Deduct") and ("Prepmt Amt to Deduct" <> 0):
                                                AddError(StrSubstNo(Text053, FieldCaption("Prepmt Amt to Deduct"), Round(Fraction * "Line Amount", GLSetup."Amount Rounding Precision")));
                                            (1 - Fraction) * "Line Amount" < "Prepmt. Amt. Inv." - "Prepmt Amt Deducted" - "Prepmt Amt to Deduct":
                                                AddError(StrSubstNo(Text054, FieldCaption("Prepmt Amt to Deduct"), Round("Prepmt. Amt. Inv." - "Prepmt Amt Deducted" - (1 - Fraction) * "Line Amount", GLSetup."Amount Rounding Precision")));
                                        end;
                                    end;
                                if not "Prepayment Line" and ("Prepmt. Line Amount" > 0) then if "Prepmt. Line Amount" > "Prepmt. Amt. Inv." then AddError(StrSubstNo(Text046, FieldCaption("Prepmt. Line Amount")));
                        ...
                        begin
                            ...
                            while MoreLines and (TempSalesLine.Description = '') and (TempSalesLine."Description 2" = '') and (TempSalesLine."No." = '') and (TempSalesLine.Quantity = 0) and (TempSalesLine.Amount = 0) do MoreLines := TempSalesLine.Next(-1) <> 0;
                    ...
                    dataitem(VATCounter; "Integer")
                    {
                        ...
                        begin
                            ...
                            CurrReport.CREATETOTALS(VATAmountLine."VAT Base", VATAmountLine."VAT Amount", VATAmountLine."Amount Including VAT", VATAmountLine."Line Amount", VATAmountLine."Inv. Disc. Base Amount", VATAmountLine."Invoice Discount Amount");
                        end;
                    }
                    ...
                    dataitem("Item Charge Assignment (Sales)"; "Item Charge Assignment (Sales)")
                    {
                        ...
                        trigger OnPreDataItem()
                        begin
                            if not ShowCostAssignment then CurrReport.Break();
            ...
            begin
                ...
                ELSE BEGIN
                    IF Cust.GET("Sell-to Customer No.") THEN BEGIN
                        IF (Cust.Blocked = Cust.Blocked::Ship) AND Ship THEN BEGIN
                            ...
                            IF SalesLine2.FINDFIRST THEN ShipQtyExist := TRUE;
                        END;
                        IF (Cust.Blocked = Cust.Blocked::All) OR ((Cust.Blocked = Cust.Blocked::Invoice) AND (NOT ("Document Type" IN ["Document Type"::"Credit Memo", "Document Type"::"Return Order"]))) OR ShipQtyExist THEN AddError(STRSUBSTNO(Text045, Cust.FIELDCAPTION(Blocked), Cust.Blocked, Cust.TABLECAPTION, "Sell-to Customer No."));
                    END
                    ELSE
                        AddError(STRSUBSTNO(Text008, Cust.TABLECAPTION, "Sell-to Customer No."));
                END;
                ...
                ELSE BEGIN
                    IF "Bill-to Customer No." <> "Sell-to Customer No." THEN
                        IF Cust.GET("Bill-to Customer No.") THEN BEGIN
                            IF (Cust.Blocked = Cust.Blocked::All) OR ((Cust.Blocked = Cust.Blocked::Invoice) AND ("Document Type" IN ["Document Type"::"Credit Memo", "Document Type"::"Return Order"])) THEN AddError(STRSUBSTNO(Text045, Cust.FIELDCAPTION(Blocked), FALSE, Cust.TABLECAPTION, "Bill-to Customer No."));
                        END
                        ELSE
                            AddError(STRSUBSTNO(Text008, Cust.TABLECAPTION, "Bill-to Customer No."));
                END;
                ...
                if not (Ship or Invoice or Receive) then AddError(StrSubstNo(Text034, FieldCaption(Ship), FieldCaption(Invoice), FieldCaption(Receive)));
                if Invoice then begin
                    SalesLine.Reset();
                    SalesLine.SetRange("Document Type", "Document Type");
                    SalesLine.SetRange("Document No.", "No.");
                    SalesLine.SetFilter(Quantity, '<>0');
                    if "Document Type" in ["Document Type"::Order, "Document Type"::"Return Order"] then SalesLine.SetFilter("Qty. to Invoice", '<>0');
                ...
                if Ship then begin
                    ...
                    SalesLine.SetRange("Document No.", "No.");
                    SalesLine.SetFilter(Quantity, '<>0');
                    if "Document Type" = "Document Type"::Order then SalesLine.SetFilter("Qty. to Ship", '<>0');
                ...
                if Receive then begin
                    ...
                    SalesLine.SetRange("Document No.", "No.");
                    SalesLine.SetFilter(Quantity, '<>0');
                    if "Document Type" = "Document Type"::"Return Order" then SalesLine.SetFilter("Return Qty. to Receive", '<>0');
                    SalesLine.SetRange("Return Receipt No.", '');
                    Receive := SalesLine.Find('-');
                end;
                if not (Ship or Invoice or Receive) then AddError(Text012);
                ...
                if Ship and ("Shipping No." = '') then // Order,Invoice
                    if ("Document Type" = "Document Type"::Order) or (("Document Type" = "Document Type"::Invoice) and SalesSetup."Shipment on Invoice") then if "Shipping No. Series" = '' then AddError(StrSubstNo(Text006, FieldCaption("Shipping No. Series")));
                if Receive and ("Return Receipt No." = '') then // Return Order,Credit Memo
                    if ("Document Type" = "Document Type"::"Return Order") or (("Document Type" = "Document Type"::"Credit Memo") and SalesSetup."Return Receipt on Credit Memo") then if "Return Receipt No. Series" = '' then AddError(StrSubstNo(Text006, FieldCaption("Return Receipt No. Series")));
                if Invoice and ("Posting No." = '') then if "Document Type" in ["Document Type"::Order, "Document Type"::"Return Order"] then if "Posting No. Series" = '' then AddError(StrSubstNo(Text006, FieldCaption("Posting No. Series")));
                SalesLine.Reset();
                SalesLine.SetRange("Document Type", "Document Type");
                SalesLine.SetRange("Document No.", "No.");
                    ...
                    if SalesLine.Find('-') then
                        repeat
                            if PurchOrderHeader."No." <> SalesLine."Purchase Order No." then begin
                                ...
                                if PurchOrderHeader."Pay-to Vendor No." = '' then AddError(StrSubstNo(Text013, PurchOrderHeader.FieldCaption("Pay-to Vendor No.")));
                                if PurchOrderHeader."Receiving No." = '' then if PurchOrderHeader."Receiving No. Series" = '' then AddError(StrSubstNo(Text013, PurchOrderHeader.FieldCaption("Receiving No. Series")));
                            end;
                        until SalesLine.Next() = 0;
                if "Document Type" in ["Document Type"::Order, "Document Type"::Invoice] then if SalesSetup."Ext. Doc. No. Mandatory" and ("External Document No." = '') then AddError(StrSubstNo(Text006, FieldCaption("External Document No.")));
                if not DimMgt.CheckDimIDComb("Dimension Set ID") then AddError(DimMgt.GetDimCombErr);
                ...
                if not DimMgt.CheckDimValuePosting(TableID, No, "Dimension Set ID") then AddError(DimMgt.GetDimValuePostingErr);
                SalesSetup.TESTFIELD("Proforma Invoice ID"); //KG
                IDNo := SalesSetup."Proforma Invoice ID";
                ...
                group(Options)
                {
                    ...
                    group("Order/Return Order Posting")
                    {
                        ...
                        field(ShipReceiveOnNextPostReq; ShipReceiveOnNextPostReq)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if not ShipReceiveOnNextPostReq then InvOnNextPostReq := true;
                            end;
                        }
                        field(InvOnNextPostReq; InvOnNextPostReq)
                        {
                            ...
                            trigger OnValidate()
                            begin
                                if not InvOnNextPostReq then ShipReceiveOnNextPostReq := true;
                            end;
                        }
                    }
    ...
    begin
        ...
        CompInfo.GET; //<<KG
        CompInfo.CALCFIELDS(Picture); //<<KG
    end;
    ...
    var
        ...
        IDNo: Code[20];
        IssueNo: Code[30];
        DocumentCaption: Label 'Sales Proforma Invoice';
        QMSIDCaption: Label 'QMS ID Number:';
            ...
            if Abs(RemQtyToBeInvoiced) > Abs("Qty. to Ship") then begin
                ...
                if SaleShptLine.Find('-') then
                    repeat
                        ...
                        if not DimMgt.CheckDimIDConsistency(TempDimSetEntry, TempPostedDimSetEntry, DATABASE::"Sales Line", DATABASE::"Sales Shipment Line") then AddError(DimMgt.GetDocDimConsistencyErr);
                        if SaleShptLine."Sell-to Customer No." <> "Sell-to Customer No." then AddError(StrSubstNo(Text024, FieldCaption("Sell-to Customer No.")));
                        if SaleShptLine.Type <> Type then AddError(StrSubstNo(Text024, FieldCaption(Type)));
                        if SaleShptLine."No." <> "No." then AddError(StrSubstNo(Text024, FieldCaption("No.")));
                        if SaleShptLine."Gen. Bus. Posting Group" <> "Gen. Bus. Posting Group" then AddError(StrSubstNo(Text024, FieldCaption("Gen. Bus. Posting Group")));
                        if SaleShptLine."Gen. Prod. Posting Group" <> "Gen. Prod. Posting Group" then AddError(StrSubstNo(Text024, FieldCaption("Gen. Prod. Posting Group")));
                        if SaleShptLine."Location Code" <> "Location Code" then AddError(StrSubstNo(Text024, FieldCaption("Location Code")));
                        if SaleShptLine."Job No." <> "Job No." then AddError(StrSubstNo(Text024, FieldCaption("Job No.")));
                        if -SalesLine."Qty. to Invoice" * SaleShptLine.Quantity < 0 then AddError(StrSubstNo(Text027, FieldCaption("Qty. to Invoice")));
                        QtyToBeInvoiced := RemQtyToBeInvoiced - SalesLine."Qty. to Ship";
                        if Abs(QtyToBeInvoiced) > Abs(SaleShptLine.Quantity - SaleShptLine."Quantity Invoiced") then QtyToBeInvoiced := -(SaleShptLine.Quantity - SaleShptLine."Quantity Invoiced");
                        RemQtyToBeInvoiced := RemQtyToBeInvoiced - QtyToBeInvoiced;
                        SaleShptLine."Quantity Invoiced" := SaleShptLine."Quantity Invoiced" - QtyToBeInvoiced;
                        SaleShptLine."Qty. Shipped Not Invoiced" := SaleShptLine.Quantity - SaleShptLine."Quantity Invoiced" until (SaleShptLine.Next() = 0) or (Abs(RemQtyToBeInvoiced) <= Abs("Qty. to Ship"))
                else
                    AddError(StrSubstNo(Text026, "Shipment Line No.", "Shipment No."));
            end;
            if Abs(RemQtyToBeInvoiced) > Abs("Qty. to Ship") then if "Document Type" = "Document Type"::Invoice then AddError(StrSubstNo(Text036, "Shipment No."));
        end;
    end;
            ...
            if Abs(RemQtyToBeInvoiced) > Abs("Return Qty. to Receive") then begin
                ...
                if ReturnRcptLine.Find('-') then
                    repeat
                        ...
                        if not DimMgt.CheckDimIDConsistency(TempDimSetEntry, TempPostedDimSetEntry, DATABASE::"Sales Line", DATABASE::"Return Receipt Line") then AddError(DimMgt.GetDocDimConsistencyErr);
                        if ReturnRcptLine."Sell-to Customer No." <> "Sell-to Customer No." then AddError(StrSubstNo(Text038, FieldCaption("Sell-to Customer No.")));
                        if ReturnRcptLine.Type <> Type then AddError(StrSubstNo(Text038, FieldCaption(Type)));
                        if ReturnRcptLine."No." <> "No." then AddError(StrSubstNo(Text038, FieldCaption("No.")));
                        if ReturnRcptLine."Gen. Bus. Posting Group" <> "Gen. Bus. Posting Group" then AddError(StrSubstNo(Text038, FieldCaption("Gen. Bus. Posting Group")));
                        if ReturnRcptLine."Gen. Prod. Posting Group" <> "Gen. Prod. Posting Group" then AddError(StrSubstNo(Text038, FieldCaption("Gen. Prod. Posting Group")));
                        if ReturnRcptLine."Location Code" <> "Location Code" then AddError(StrSubstNo(Text038, FieldCaption("Location Code")));
                        if ReturnRcptLine."Job No." <> "Job No." then AddError(StrSubstNo(Text038, FieldCaption("Job No.")));
                        if SalesLine."Qty. to Invoice" * ReturnRcptLine.Quantity < 0 then AddError(StrSubstNo(Text039, FieldCaption("Qty. to Invoice")));
                        QtyToBeInvoiced := RemQtyToBeInvoiced - SalesLine."Return Qty. to Receive";
                        if Abs(QtyToBeInvoiced) > Abs(ReturnRcptLine.Quantity - ReturnRcptLine."Quantity Invoiced") then QtyToBeInvoiced := ReturnRcptLine.Quantity - ReturnRcptLine."Quantity Invoiced";
                        RemQtyToBeInvoiced := RemQtyToBeInvoiced - QtyToBeInvoiced;
                    ...
                    AddError(StrSubstNo(Text025, "Return Receipt Line No.", "Return Receipt No."));
            end;
            if Abs(RemQtyToBeInvoiced) > Abs("Return Qty. to Receive") then if "Document Type" = "Document Type"::"Credit Memo" then AddError(StrSubstNo(Text037, "Return Receipt No."));
        end;
    end;
    ...
    begin
        with "Sales Header" do begin
            ...
            if SalesLine.IsEmpty() then exit(false);
            if Ship then begin
                SalesLine.SetFilter("Qty. to Ship", '<>%1', 0);
                if not SalesLine.IsEmpty() then exit(true);
            end;
            if Receive then begin
    ...
    begin
        ...
        with SalesLine do begin
            ...
            "Dimension Set ID" := DimMgt.GetDefaultDimID(TableID, No, SourceCodeSetup.Sales, "Shortcut Dimension 1 Code", "Shortcut Dimension 2 Code", "Dimension Set ID", DATABASE::Customer);
        end;
    end;

    procedure InitializeRequest(NewShipReceiveOnNextPostReq: Boolean;
    ...
    var
        ErrorText: Text[250];
    begin
        with SalesLine2 do case Type of
                               Type::"G/L Account":
                                   begin
                                       if ("No." = '') and (Amount = 0) then exit;
                                       if "No." <> '' then
                                           if GLAcc.Get("No.") then begin
                                               if GLAcc.Blocked then AddError(StrSubstNo(Text007, GLAcc.FieldCaption(Blocked), false, GLAcc.TableCaption, "No."));
                                               if not GLAcc."Direct Posting" and ("Line No." <= OrigMaxLineNo) then AddError(StrSubstNo(Text007, GLAcc.FieldCaption("Direct Posting"), true, GLAcc.TableCaption, "No."));
                                           end
                                           else
                               ...
                               Type::Item:
                                   begin
                                       if ("No." = '') and (Quantity = 0) then exit;
                                       if "No." <> '' then
                                           if Item.Get("No.") then begin
                                               if Item.Blocked then AddError(StrSubstNo(Text007, Item.FieldCaption(Blocked), false, Item.TableCaption, "No."));
                                               if Item.Reserve = Item.Reserve::Always then begin
                                                   CalcFields("Reserved Quantity");
                                                   if "Document Type" in ["Document Type"::"Return Order", "Document Type"::"Credit Memo"] then begin
                                                       if (SignedXX(Quantity) < 0) and (Abs("Reserved Quantity") < Abs("Return Qty. to Receive")) then AddError(StrSubstNo(Text015, FieldCaption("Reserved Quantity"), SignedXX("Return Qty. to Receive")));
                                                   end
                                                   else
                                                       if (SignedXX(Quantity) < 0) and (Abs("Reserved Quantity") < Abs("Qty. to Ship")) then AddError(StrSubstNo(Text015, FieldCaption("Reserved Quantity"), SignedXX("Qty. to Ship")));
                                               end
                                           end
                                           else
                               ...
                               Type::Resource:
                                   begin
                                       ...
                                       if Res.Get("No.") then begin
                                           if Res."Privacy Blocked" then AddError(StrSubstNo(Text007, Res.FieldCaption("Privacy Blocked"), false, Res.TableCaption, "No."));
                                           if Res.Blocked then AddError(StrSubstNo(Text007, Res.FieldCaption(Blocked), false, Res.TableCaption, "No."));
                                       end
                                       else
                                   ...
                                   begin
                                       ...
                                       if "No." <> '' then
                                           if FA.Get("No.") then begin
                                               if FA.Blocked then AddError(StrSubstNo(Text007, FA.FieldCaption(Blocked), false, FA.TableCaption, "No."));
                                               if FA.Inactive then AddError(StrSubstNo(Text007, FA.FieldCaption(Inactive), false, FA.TableCaption, "No."));
                                               if "Depreciation Book Code" = '' then
                                                   AddError(StrSubstNo(Text006, FieldCaption("Depreciation Book Code")))
                                               else
                                                   if not FADeprBook.Get("No.", "Depreciation Book Code") then AddError(StrSubstNo(Text017, FADeprBook.TableCaption, "No.", "Depreciation Book Code"));
                                           end
                                           else
                                               AddError(StrSubstNo(Text008, FA.TableCaption, "No."));
                                   end;
                               else begin
                                       OnCheckSalesLineCaseTypeElse(Type.AsInteger(), "No.", ErrorText);
                                       if ErrorText <> '' then AddError(ErrorText);
                                   end;
            end;
    end;
        ...
        with SalesHeader do begin
            if "Sell-to Customer No." = '' then
                ...
                if Cust.Get("Sell-to Customer No.") then begin
                    ...
                    if Cust."Privacy Blocked" then AddError(Cust.GetPrivacyBlockedGenericErrorText(Cust));
                    if (Cust.Blocked = Cust.Blocked::All) or ((Cust.Blocked = Cust.Blocked::Invoice) and (not ("Document Type" in ["Document Type"::"Credit Memo", "Document Type"::"Return Order"]))) or ShipQtyExist then AddError(StrSubstNo(Text045, Cust.FieldCaption(Blocked), Cust.Blocked, Cust.TableCaption, "Sell-to Customer No."))
                end
                else
                    AddError(StrSubstNo(Text008, Cust.TableCaption, "Sell-to Customer No."));
    ...
    begin
        with SalesHeader do if "Bill-to Customer No." = '' then
                AddError(StrSubstNo(Text006, FieldCaption("Bill-to Customer No.")))
            else begin
                if "Bill-to Customer No." <> "Sell-to Customer No." then
                    if Cust.Get("Bill-to Customer No.") then begin
    ...
    begin
        with SalesHeader do if "Posting Date" = 0D then
                AddError(StrSubstNo(Text006, FieldCaption("Posting Date")))
            else
                if "Posting Date" <> NormalDate("Posting Date") then
                    AddError(StrSubstNo(Text009, FieldCaption("Posting Date")))
                else begin
                    if not UserSetupManagement.TestAllowedPostingDate("Posting Date", TempErrorText) then AddError(TempErrorText);
                    if IsInvtPosting then begin
                        InvtPeriodEndDate := "Posting Date";
                        if not InvtPeriod.IsValidDate(InvtPeriodEndDate) then AddError(StrSubstNo(Text010, Format("Posting Date")))
                    end;
                end;
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCheckSalesDoc(SalesHeader: Record "Sales Header";
    var ErrorText: array[99] of Text[250];
    var ErrorCounter: Integer)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterSalesPostGetSalesLines(var SalesHeader: Record "Sales Header";
    var TempSalesLine: Record "Sales Line" temporary)
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterCreateDimTableIDs(var SalesLine: Record "Sales Line";
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckDimValuePostingHeader(var SalesHeader: Record "Sales Header";
    var TableID: array[10] of Integer;
    var No: array[10] of Code[20]);
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnBeforeCheckDimValuePostingLine(var SalesLine: Record "Sales Line";
    var TableID: array[10] of Integer;
    var No: array[10] of Code[20]);
    begin
    end;
    ...
    [IntegrationEvent(false, false)]
    local procedure OnCheckSalesLineCaseTypeElse(LineType: Option;
    "No.": Code[20];
    var ErrorText: Text[250])
    begin
    end;

    [IntegrationEvent(false, false)]
    local procedure OnAfterGetRecordSalesLineOnBeforeCheckDim(SalesLine: Record "Sales Line";
