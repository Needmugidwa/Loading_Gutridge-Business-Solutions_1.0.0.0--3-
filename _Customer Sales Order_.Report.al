report 50214 "Customer Sales Order"
{
    DefaultLayout = RDLC;
    RDLCLayout = './Customer - Sales Order.rdl';
    ...
    dataset
    {
        ...
        {
            DataItemTableView = SORTING("Document Type", "No.")WHERE("Document Type"=CONST(Quote), "Document Type"=Const(Order));
            ...
            column(ForeignBank_Name;ForeignBank.Name)
            {
            }
            column(ForeignBank_Acc;ForeignBank."Bank Account No.")
            {
            }
            column(ForeignBank_Branch;ForeignBank."Bank Branch No.")
            {
            }
            column(ForeignBank_Swift_Code;ForeignBank."SWIFT Code")
            {
            }
            column(LocalBank_Name;LocalBank.Name)
            {
            }
            column(LocalBank_Acc;LocalBank."Bank Account No.")
            {
            }
            column(LocalBank_Branch;LocalBank."Bank Branch No.")
            {
            }
            column(LocalBank_Swift;LocalBank."SWIFT Code")
            {
            }
            ...
            dataitem(CopyLoop;"Integer")
            {
                ...
                dataitem(PageLoop;"Integer")
                {
                    ...
                    column(Global_Order_No;"Sales Header"."Blanket Order No.")
                    {
                    }
                    column(Global_Order_No_Caption;GlobalOrderNoCaption)
                    {
                    }
                    ...
                    column(Blanket_Order_No;"Sales Header"."Blanket Order No.")
                    {
                    }
                ...
                trigger OnAfterGetRecord()var PrepmtSalesLine: Record "Sales Line" temporary;
            ...
            trigger OnAfterGetRecord()var SalesSetup: Record "Sales & Receivables Setup";
            begin
                //CurrReport.Language := Language.GetLanguageID("Language Code");
                //MESSAGE("Sales Header"."Blanket Order No.");
                ...
                SalesSetup.Get;
                Qms:=SalesSetup."S-Order ID";
            end;
            trigger OnPreDataItem()begin
                Print:=Print or not CurrReport.Preview;
                AsmInfoExistsForLine:=false;
                //>>show bank details 240317
                SalesSetup.Get;
                //SalesSetup.TESTFIELD(SalesSetup."Bank on Quotes(Foreign)");
                CompInfo.Get;
                SalesSetup.TestField(SalesSetup."Bank on Quotes(Local)");
                LocalBank.Get(SalesSetup."Bank on Quotes(Local)");
                ForeignBank.Get(SalesSetup."Bank on Quotes(Foreign)");
            //>>show bank details 240317
            end;
        }
    }
    ...
    var Text004: Label 'Order Confirmation %1', Comment='%1 = Document No.';
    ...
    LocalBank: Record "Bank Account";
    ForeignBank: Record "Bank Account";
    CompInfo: Record "Company Information";
    Qms: Code[10];
    IssueDateCaption: Label 'ISSUE DATE:';
    IssueNo: Code[30];
    GlobalOrderNoCaption: Label 'Global Order No.';
    ...
    local procedure GetUnitOfMeasureDescr(UOMCode: Code[10]): Text[10]var UnitOfMeasure: Record "Unit of Measure";
    begin
